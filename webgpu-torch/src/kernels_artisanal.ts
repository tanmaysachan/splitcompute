...
`
    ...
    var bIndex = outputBatch * parameters.bBatchStride + outputRow * parameters.bRowStride + outputCol;

    if (b[bIndex] != 0.0) {
        result = parameters.fill;
    } else {
        result = a[aIndex];
    }
...
`
    ...
    // Need to have the hacky second condition, I'm likely doing something wrong
    if (outputRow + parameters.diagonal < outputCol || outputRow < 0 - parameters.diagonal) {
...
`
    ...
    triu: {
        // TODO: Negative diagonals are broken, some webgpu kink
        name: "triu",
        config: [
            {
                name: "dtype",
    ...
    if (outputRow + parameters.diagonal > outputCol) {
        output[outputIndex] = 0.0;
        return;
    }
    output[outputIndex] = a[aIndex];
